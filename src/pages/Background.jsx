...
import bgImg from "../assets/background/background_background.webp";
...
const Background = () => {
  ...
  return (
    <div className="background">
      ...
      />
      <div className={`sidebar${open && !isClosing ? "-open" : open && isClosing ? "-close" : ""}`}>
        <div className="open-btn" onClick={handleOpen}>
          {btnText}
        </div>
        {open && (
          <>
            <div className={`sidebar-title${isClosing ? "-close" : ""}`}>프로젝트 소개</div>
            <div className={`sidebar-content${isClosing ? "-close" : ""}`}>
              {sidebarText.map((text, index) => (
                <div
                  key={index}
                  className={`sidebar-text${
                    index === 0 || index % 2 === 1 ? "" : "-normal"
                  }${isClosing ? "-close" : ""}`}
                >
                  {text}
                </div>
            ...
            <div className={`sidebar-bottom${isClosing ? "-close" : ""}`}>
              {sidebarBottomText.map((text, index) => (
                <div
                  key={index}
                  className={`sidebar-bottom-text${index === 0 ? "-name" : ""}${isClosing ? "-close" : ""}`}
                >
                  {text}
                </div>
              ))}
      ...
      <div className="bg-container">
        <img className="letter" src={letter} />
        <div className="letter-box">
          <img className="letter-text" src={letter_text} />
        </div>
      </div>
    </div>
  );
